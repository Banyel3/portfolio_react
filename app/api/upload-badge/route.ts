...
const BUCKET = process.env.SUPABASE_BUCKET_NAME ?? "badges";
...
export async function POST(req: Request) {
  try {
    ...
    if (!publicUrl) {
      ...
      if (signedErr) {
        console.error("[upload-badge] createSignedUrl error", signedErr);
    ...
    console.error("[upload-badge] POST error", err);
    return NextResponse.json(
...
export async function DELETE(req: Request) {
  try {
    ...
    if (error) {
      console.error("[upload-badge] DELETE error", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("[upload-badge] DELETE error", err);
    return NextResponse.json(
      { error: String(err?.message || err) },
      { status: 500 }
    );
