...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch certificates", details: String(error) }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to create certificate", details: String(error) }, { status: 500 })
  }
}
